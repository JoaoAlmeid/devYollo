...
import { getCurrentScope, addEventProcessor, makeSession, updateSession, getClient, captureEvent, flush } from '@sentry/core';
...
function startChildProcess(options) {
  ...
  try {
    ...
    const timer = setInterval(() => {
      try {
        const currentSession = _optionalChain([getCurrentScope, 'call', _2 => _2(), 'optionalAccess', _3 => _3.getSession, 'call', _4 => _4()]);
    ...
    child.on('message', (msg) => {
      if (msg === 'session-ended') {
        ...
        _optionalChain([getCurrentScope, 'call', _5 => _5(), 'optionalAccess', _6 => _6.setSession, 'call', _7 => _7(undefined)]);
      }
    });
...
function handleChildProcess(options) {
  ...
  function sendAnrEvent(frames) {
    if (session) {
      ...
      _optionalChain([getClient, 'call', _8 => _8(), 'optionalAccess', _9 => _9.sendSession, 'call', _10 => _10(session)]);

      try {
        // Notify the main process that the session has ended so the session can be cleared from the scope
        _optionalChain([process, 'access', _11 => _11.send, 'optionalCall', _12 => _12('session-ended')]);
      } catch (_) {
        // ignore
      }
    }
